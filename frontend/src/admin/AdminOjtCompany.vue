...
import '../assets/admin/AdminOjtCompany.css'
...
const form = ref({ company_name: '', description: '', contact_number: '', email_address: '', address: '' })
...
function showMessage(type, message) {
  ...
  messageTimer = setTimeout(() => { errorMessage.value = ''; successMessage.value = '' }, 3000)
}
...
function resetForm() {
  ...
  form.value = { company_name: '', description: '', contact_number: '', email_address: '', address: '' }
}

function openCreate() { resetForm(); isFormOpen.value = true }
...
async function saveCompany() {
  ...
  isSaving.value = true
  try {
    if (isUpdate) await api.put(`/api/companies/${editingId.value}`, form.value)
    else await api.post('/api/companies', form.value)
...
async function deleteCompany(company) {
  ...
  try {
    ...
    showMessage('success', 'OJT company deleted successfully.')
    await loadCompanies()
  } catch (error) {
    showMessage('error', apiError(error))
  }
  ...
  <div class="admin-layout">
    ...
    <main class="company-page">
      <header class="page-header">
        <div>
          ...
          <h1>OJT companies</h1>
          <p>Manage company records used for student placements.</p>
        </div>
      ...
      <table>
        <thead><tr><th>Company</th><th>Description</th><th>Contact</th><th>Email</th><th>Address</th><th>Actions</th></tr></thead>
        <tbody>
          <tr v-for="company in companies" :key="company.company_id">
            ...
            <td class="actions">
              <button type="button" @click="openEdit(company)">Edit</button>
              <button type="button" @click="deleteCompany(company)">Delete</button>
            </td>
          </tr>
    ...
    <div v-if="isFormOpen" class="modal-backdrop" @click.self="closeForm">
      <form class="modal" @submit.prevent="saveCompany">
        <h2>{{ formTitle }}</h2>
        <label>Company name<input v-model.trim="form.company_name" required maxlength="150" /></label>
        <label>Contact number<input v-model.trim="form.contact_number" maxlength="30" /></label>
        <label>Email address<input v-model.trim="form.email_address" type="email" maxlength="100" /></label>
        <label>Address<input v-model.trim="form.address" required maxlength="255" /></label>
        <label>Description<textarea v-model.trim="form.description" rows="5"></textarea></label>
...
@media (max-width: 700px) { .admin-layout { display: block; } .company-page { padding: 24px 16px; overflow-x: auto; } .page-header { align-items: start; flex-direction: column; } }
</style>
